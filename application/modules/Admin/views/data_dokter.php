                    ...
                    <div class="body">
                        <div class="table-responsive">
                            <table class="table table-bordered table-striped table-hover js-basic-example dataTable" id="data-user">
                                ...
                                <tbody>
                                    <?php $no=1; foreach ($dataDok as $data) { ?>
                                        <tr>
                                            <td><?= $no++ ?></td>
                                            ...
                                            <td>
                                                <div class="row">
                                                    <div class="col-lg-12 col-md-12 col-sm-12">
                                                        ...
                                                        <button onclick="ubahStatus(<?= $data->id_user ?>, 'aktif')" <?= ($data->status == 'aktif' || $data->status == 'ditolak'?'disabled':'') ?> title="Disetujui" class="btn cbtn-raised <?= ($data->status == 'aktif' || $data->status == 'ditolak'?'btn-secondary':'bg-green') ?> waves-effect" style="margin-bottom: 5px;"><i class="material-icons" style="margin-top: -5px">check_circle</i></button>

                                                        <button <?= ($data->status == 'aktif' || $data->status == 'ditolak'?'disabled':'') ?> title="Ditolak" class="btn cbtn-raised <?= ($data->status == 'aktif' || $data->status == 'ditolak'?'btn-secondary':'btn-danger') ?> waves-effect" onclick="ubahStatus(<?= $data->id_user ?>, 'ditolak')" style="margin-bottom: 5px;"><i class="material-icons" style="margin-top: -5px">cancel</i></button>
...
<script type="text/javascript">
    function ubahStatus(id='', status='') {
        ...
        swal({
          title: "Permintaan sebagai Dokter",
          text: "Apakah data user akan "+sts+" sebagai Dokter?",
          type: "info",
          ...
          showLoaderOnConfirm: true,
            ...
            setTimeout(function () {
                    ...
                    $.post("<?= base_url() ?>/Admin/ubahStatusDokter", {id_user: id, status: status}, function(result){
                      // alert(result);
                        if (result == 'Success') {
                            swal("Data berhasil disimpan!");  
                            window.location.href = "<?= base_url().'Admin/dataDokter/' ?>";
                        }else{
                            swal("Gagal!");
                        }
                    });
